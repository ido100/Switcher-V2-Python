	...
	try:
		...
		while True:
			...
			if ba.hexlify(data)[0:4] != "fef0" and len(data) != 165:
				...
				print("[+] Switcher Name: " + data[42:74].decode().rstrip('\x00'))
...
#	import pdb;pdb.set_trace()
	data = b"fef052000232a100" + pSession + b"340001000000000000000000"  + getTS() + b"00000000000000000000f0fe1c00" + phone_id + b"0000" + device_pass + b"00000000000000000000000000000000000000000000000000000000"
	data = crcSignFullPacketComKey(data, pKey)
	print ("[*] Sending Login Packet to Switcher...")
	s.send(ba.unhexlify(data))
	res = s.recv(1024)
	...
	if not pSession2:
		...
		print(("[+] Received SessionID: " + pSession2.decode()))

	data = b"fef0300002320103" + pSession2 + b"340001000000000000000000" + getTS() + b"00000000000000000000f0fe" + device_id + b"00"
	data = crcSignFullPacketComKey(data, pKey)
...
#		import pdb;pdb.set_trace()
		data = b"fef05d0002320102" + pSession2 + b"340001000000000000000000" + getTS() + b"00000000000000000000f0fe" + device_id + b"00" + phone_id + b"0000" + device_pass + b"000000000000000000000000000000000000000000000000000000000106000" + bytes(sCommand, 'utf-8') + b"0000000000"
		data = crcSignFullPacketComKey(data, pKey)
		if sCommand == "0":
